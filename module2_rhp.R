...
# nmf_w_basis_ccle: "module1/nmf_w_basis_ccle.RDS"
# nmf_w_basis_tumor: "module1/nmf_w_basis_tumor.RDS"
...
nmf_w_basis_ccle_path <- "https://www.dropbox.com/s/ipa3l1anyqf424e/nmf_w_basis_ccle.RDS?dl=1"
nmf_w_basis_tumor_path <- "https://www.dropbox.com/s/hn1b8wu90ivlny1/nmf_w_basis_tumor.RDS?dl=1"

# read scRNA-seq data from cell lines and tumors
expr_ccle <- readRDS(url(cellline_data_path))
expr_tumor <- readRDS(url(tumor_data_path)) # human tumors

# read metadata
meta_ccle <- readRDS("smaller_data_files/CCLE_metadata.RDS")
...
nmf_programs_genes_ccle <- readRDS(url(nmf_w_basis_ccle_path)) # nmf gene scores
nmf_programs_cells_ccle <- readRDS("Expected_results/module1/nmf_h_coef_ccle.RDS") # nmf cell scores

nmf_programs_genes_tumor <- readRDS(url(nmf_w_basis_tumor_path)) # nmf gene scores
...
# combine p1 plots
pdf(file.path(OUTPUT_DIR,"module2/dbscan_metaprograms_eps1.8_ccle.pdf"), height = 6.5, width = 9, onefile = F)
...
# combine p2 plots
pdf(file.path(OUTPUT_DIR,"module2/nmf_metaprograms_complexity_ccle.pdf"), height = 6, width = 7, onefile=FALSE)
...
saveRDS(nmf_programs_sig_ccle[,colnames(nmf_intersect_ccle)], file.path(OUTPUT_DIR,"module2/nmf_programs_sig_ccle.RDS"))
...
# combine p3 plots
pdf(file.path(OUTPUT_DIR,"module2/nmf_metaprograms_discrete_ccle.pdf"), height = 6, width = 7, onefile=FALSE)
...
saveRDS(nmf_programs_sig_ccle[,colnames(nmf_intersect_nc_ccle)], file.path(OUTPUT_DIR,"module2/nmf_programs_sig_nc_ccle.RDS"))
...
saveRDS(nmf_meta_all_ccle,file.path(OUTPUT_DIR,"module2/nmf_metaprograms_sig_nc_ccle.RDS"))

saveRDS(lapply(nmf_meta_all_ccle, function(x) x[x>=0.25]),file.path(OUTPUT_DIR,"module2/nmf_metaprograms_sigtop25_nc_ccle.RDS"))
...
saveRDS(list(skinpig=nmf_meta1_ccle_programs, emtI=nmf_meta2_ccle_programs, emtII=nmf_meta3_ccle_programs, ifn=nmf_meta4_ccle_programs,
            ...
            protmat=nmf_meta9_ccle_programs, protdreg=nmf_meta10_ccle_programs),
        file.path(OUTPUT_DIR,"module2/nmf_metaprograms_programs_nc_ccle.RDS"))
...
## Error here; something not right; unclear what format should be ##
...
# combine plots 4
pdf(file.path(OUTPUT_DIR,"module2/nmf_metaprograms_nc_ccle.pdf"), height = 9.5, width = 8, onefile=FALSE)
...
p5.3 <- ggplot(nmf_corr_complexity_tumor, aes(y=corr, x = 1:nrow(nmf_corr_complexity_tumor))) +
  ...
  scale_x_continuous(expand = c(0,0))
# combine plots p5
pdf(file.path(OUTPUT_DIR,"module2/nmf_metaprograms_tumors.pdf"), width = 9.5, height = 7, onefile = F)
...
saveRDS(nmf_meta_all_tumor,file.path(OUTPUT_DIR,"module2/nmf_metaprograms_sig_nc_tumor.RDS"))

saveRDS(lapply(nmf_meta_all_tumor, function(x) x[x>=0.25]),file.path(OUTPUT_DIR,"module2/nmf_metaprograms_sigtop25_nc_tumor.RDS"))

saveRDS(list(episen=nmf_meta1_tumor_programs, immu=nmf_meta2_tumor_programs, stress=nmf_meta3_tumor_programs, hypoxia=nmf_meta4_tumor_programs, emt=nmf_meta5_tumor_programs),file.path(OUTPUT_DIR,"module2/nmf_metaprograms_programs_nc_tumor.RDS"))
